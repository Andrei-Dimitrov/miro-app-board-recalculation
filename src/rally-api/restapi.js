...
const refUtils = require("./util/ref");
    ...
    options = _.merge(
      {
        ...
        requestOptions: {
          ...
          gzip: true,
          headers: {
            "X-RallyIntegrationLibrary": `rally-node v2.0`,
            "X-RallyIntegrationName": "Rally Node bindings",
            "X-RallyIntegrationVendor": "Rally Software, Inc.",
            "X-RallyIntegrationVersion": "2.0",
          },
        },
      },
      options,
      ...
      options = _.merge(
        {
          requestOptions: {
            auth: {
              user:
                (options && (options.user || options.userName)) ||
                process.env.RALLY_USERNAME,
              pass:
                (options && (options.pass || options.password)) ||
                process.env.RALLY_PASSWORD,
  ...
  query(options) {
    ...
    const requestOptions = _.merge(
      {
        ...
        qs: {
          ...
          pagesize: options.limit
            ? Math.min(options.pageSize, options.limit)
            : options.pageSize,
        },
      },
      options.requestOptions,
    ...
    if (options.query) {
      requestOptions.qs.query =
        (options.query.toQueryString && options.query.toQueryString()) ||
        options.query;
    }
    ...
    function loadRemainingPages(result) {
      ...
      if (
        options.limit &&
        result.StartIndex + options.pageSize <=
          Math.min(options.limit || options.pageSize, result.TotalResultCount)
      ) {
        return self.request
          .get(
